...
export default function AskAI() {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      setError('Could not reach the AI assistant right now — try the contact section instead.')
    } finally {
      setLoading(false)
    }
  }
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-[340px] max-w-[calc(100vw-3rem)] rounded-lg border border-border bg-elevated shadow-xl shadow-black/40 flex flex-col overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-elevated2">
            ...
            <span className="ml-auto font-mono text-[10px] text-muted">beta</span>
          </div>

          <div ref={scrollRef} className="flex-1 max-h-[360px] overflow-y-auto px-4 py-4 space-y-3">
            {messages.map((m, i) => (
            ...
            {error && <div className="text-xs text-amber font-mono">{error}</div>}
          </div>
